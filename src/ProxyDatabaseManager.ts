...
/**
 * 高級數據庫管理器 - 使用 better-sqlite3 with enhanced features
 */
export class ProxyDatabaseManager {
  private db: Database.Database;
  private logger: Logger;
  
  // Prepared statements for better performance
  private insertImageStmt!: Database.Statement<any[]>;
  private updateStatusStmt!: Database.Statement<any[]>;
  private selectUrlStmt!: Database.Statement<[string]>;
  private insertKeywordStmt!: Database.Statement<any[]>;
  ...
  constructor(dbPath: string = './data/anime_collection.db') {
    ...
    this.logger.info(`Proxy database initialized: ${dbPath}`);
  }

  /**
   * 配置數據庫性能 - better-sqlite3-proxy 風格的優化
   */
  private configureDatabase(): void {
    ...
    this.logger.info('Database optimized with proxy-like configuration');
  }
  ...
  /**
   * 預編譯 SQL 語句以提升性能 (類似 better-sqlite3-proxy 的優化)
   */
  private prepareStatements(): void {
    this.insertImageStmt = this.db.prepare(`
      INSERT INTO images (
    ...
    this.selectUrlStmt = this.db.prepare(`
    ...
    this.logger.info('SQL statements prepared for optimal performance');
  }

  /**
   * 插入關鍵字配置 (使用 proxy-style batch operations)
   */
  public insertKeywords(keywords: CharacterKeyword[]): void {
    const transaction = this.db.transaction((keywords: CharacterKeyword[]) => {
      for (const kw of keywords) {
        this.insertKeywordStmt.run(
          kw.keyword, kw.category, kw.subcategory, 
          kw.targetCount, kw.relevanceScore
        );
      }
    });
    ...
    try {
      ...
      this.logger.info(`Inserted ${keywords.length} keywords with proxy-style transaction`);
    } catch (error) {
  ...
  public async insertImage(record: ImageRecord): Promise<number> {
    try {
      const result = this.insertImageStmt.run(
        record.keyword,
        record.category,
        record.subcategory,
        record.originalUrl,
  ...
  /**
   * 批量插入圖片記錄 (transaction-based like proxy)
   */
  public async insertImagesBatch(records: ImageRecord[]): Promise<number[]> {
    const insertTransaction = this.db.transaction((records: ImageRecord[]) => {
    ...
    try {
      const results = insertTransaction(records);
  ...
  /**
   * 檢查 URL 是否已存在 (使用預編譯語句)
   */
  public urlExists(originalUrl: string): boolean {
    try {
      const result = this.selectUrlStmt.get(originalUrl);
      return !!result;
    } catch (error) {
  ...
  /**
   * 獲取收集統計 (proxy-enhanced analytics)
   */
  ...
  public close(): void {
    try {
      this.db.close();
      this.logger.info('Proxy database connection closed');
    } catch (error) {
